(function ($) {
  ...
  var halo = {
    ...
    initColorTheme: function () {
      ...
      //   window.matchMedia("(prefers-color-scheme: dark)").matches;
       const storedTheme = localStorage.getItem("plantlab-theme");
      ...
      const toggleTheme = () => {
        ...
        document.documentElement.setAttribute("data-theme", theme);
        localStorage.setItem("plantlab-theme", theme);
      };

      if (theme == "dark" && !document.querySelector("#chk").checked)
        document.querySelector("#chk").checked = true;
      if (theme == "light" && document.querySelector("#chk").checked)
        document.querySelector("#chk").checked = false;

      document.querySelector("#chk").addEventListener("change", toggleTheme);
    },
      ...
      if (enable) {
        ...
        const setImageForMetafields = (list, unfilter) => {
          ...
          array.forEach((element) => {
            slidesToKeep.push(
              `.filter-${element.toLowerCase().replace(/\s/g, "-")}`
            );
          });
  ...
  window.updateCart = function (cart) {
    halo.updateSidebarCart(cart);
    document.body.classList.add("cart-sidebar-show"); // Confirme no seu tema se halo.updateSidebarCart é a função que atualiza o carrinho
  };

  // Captura o parametro da URL atual
  var queryString = window.location.search;
  var searchParams = new URLSearchParams(queryString);

  // Verifique se o parâmetro 'cart-show' está presente
  var cartOpen = searchParams.has("cart-show");
